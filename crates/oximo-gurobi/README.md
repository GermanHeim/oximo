...
```sh
cargo run -p oximo --example gurobi --features gurobi
```

## Options

`GurobiOptions` is a typed builder. Universal and MIP options use the same trait-based API as other backends. Gurobi-specific parameters are generated by a macro, each method corresponds to a Gurobi parameter (snake_case of the official name).
